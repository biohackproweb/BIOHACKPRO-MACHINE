/* eslint-disable */

// @ts-nocheck

// noinspection JSUnusedGlobalSymbols

// This file was automatically generated by TanStack Router.
// You should NOT make any changes in this file as it will be overwritten.
// Additionally, you should also exclude this file from your linter and/or formatter to prevent it from being checked or modified.

import { Route as rootRouteImport } from './routes/__root'
import { Route as GarantiaSoporteRouteImport } from './routes/garantia-soporte'
import { Route as ContactoRouteImport } from './routes/contacto'
import { Route as IndexRouteImport } from './routes/index'
import { Route as MaquinasIndexRouteImport } from './routes/maquinas.index'
import { Route as CategoriasIndexRouteImport } from './routes/categorias.index'
import { Route as MaquinasSlugRouteImport } from './routes/maquinas.$slug'
import { Route as CategoriasSlugRouteImport } from './routes/categorias.$slug'

const GarantiaSoporteRoute = GarantiaSoporteRouteImport.update({
  id: '/garantia-soporte',
  path: '/garantia-soporte',
  getParentRoute: () => rootRouteImport,
} as any)
const ContactoRoute = ContactoRouteImport.update({
  id: '/contacto',
  path: '/contacto',
  getParentRoute: () => rootRouteImport,
} as any)
const IndexRoute = IndexRouteImport.update({
  id: '/',
  path: '/',
  getParentRoute: () => rootRouteImport,
} as any)
const MaquinasIndexRoute = MaquinasIndexRouteImport.update({
  id: '/maquinas/',
  path: '/maquinas/',
  getParentRoute: () => rootRouteImport,
} as any)
const CategoriasIndexRoute = CategoriasIndexRouteImport.update({
  id: '/categorias/',
  path: '/categorias/',
  getParentRoute: () => rootRouteImport,
} as any)
const MaquinasSlugRoute = MaquinasSlugRouteImport.update({
  id: '/maquinas/$slug',
  path: '/maquinas/$slug',
  getParentRoute: () => rootRouteImport,
} as any)
const CategoriasSlugRoute = CategoriasSlugRouteImport.update({
  id: '/categorias/$slug',
  path: '/categorias/$slug',
  getParentRoute: () => rootRouteImport,
} as any)

export interface FileRoutesByFullPath {
  '/': typeof IndexRoute
  '/contacto': typeof ContactoRoute
  '/garantia-soporte': typeof GarantiaSoporteRoute
  '/categorias/$slug': typeof CategoriasSlugRoute
  '/maquinas/$slug': typeof MaquinasSlugRoute
  '/categorias/': typeof CategoriasIndexRoute
  '/maquinas/': typeof MaquinasIndexRoute
}
export interface FileRoutesByTo {
  '/': typeof IndexRoute
  '/contacto': typeof ContactoRoute
  '/garantia-soporte': typeof GarantiaSoporteRoute
  '/categorias/$slug': typeof CategoriasSlugRoute
  '/maquinas/$slug': typeof MaquinasSlugRoute
  '/categorias': typeof CategoriasIndexRoute
  '/maquinas': typeof MaquinasIndexRoute
}
export interface FileRoutesById {
  __root__: typeof rootRouteImport
  '/': typeof IndexRoute
  '/contacto': typeof ContactoRoute
  '/garantia-soporte': typeof GarantiaSoporteRoute
  '/categorias/$slug': typeof CategoriasSlugRoute
  '/maquinas/$slug': typeof MaquinasSlugRoute
  '/categorias/': typeof CategoriasIndexRoute
  '/maquinas/': typeof MaquinasIndexRoute
}
export interface FileRouteTypes {
  fileRoutesByFullPath: FileRoutesByFullPath
  fullPaths:
    | '/'
    | '/contacto'
    | '/garantia-soporte'
    | '/categorias/$slug'
    | '/maquinas/$slug'
    | '/categorias/'
    | '/maquinas/'
  fileRoutesByTo: FileRoutesByTo
  to:
    | '/'
    | '/contacto'
    | '/garantia-soporte'
    | '/categorias/$slug'
    | '/maquinas/$slug'
    | '/categorias'
    | '/maquinas'
  id:
    | '__root__'
    | '/'
    | '/contacto'
    | '/garantia-soporte'
    | '/categorias/$slug'
    | '/maquinas/$slug'
    | '/categorias/'
    | '/maquinas/'
  fileRoutesById: FileRoutesById
}
export interface RootRouteChildren {
  IndexRoute: typeof IndexRoute
  ContactoRoute: typeof ContactoRoute
  GarantiaSoporteRoute: typeof GarantiaSoporteRoute
  CategoriasSlugRoute: typeof CategoriasSlugRoute
  MaquinasSlugRoute: typeof MaquinasSlugRoute
  CategoriasIndexRoute: typeof CategoriasIndexRoute
  MaquinasIndexRoute: typeof MaquinasIndexRoute
}

declare module '@tanstack/react-router' {
  interface FileRoutesByPath {
    '/garantia-soporte': {
      id: '/garantia-soporte'
      path: '/garantia-soporte'
      fullPath: '/garantia-soporte'
      preLoaderRoute: typeof GarantiaSoporteRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/contacto': {
      id: '/contacto'
      path: '/contacto'
      fullPath: '/contacto'
      preLoaderRoute: typeof ContactoRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/': {
      id: '/'
      path: '/'
      fullPath: '/'
      preLoaderRoute: typeof IndexRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/maquinas/': {
      id: '/maquinas/'
      path: '/maquinas'
      fullPath: '/maquinas/'
      preLoaderRoute: typeof MaquinasIndexRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/categorias/': {
      id: '/categorias/'
      path: '/categorias'
      fullPath: '/categorias/'
      preLoaderRoute: typeof CategoriasIndexRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/maquinas/$slug': {
      id: '/maquinas/$slug'
      path: '/maquinas/$slug'
      fullPath: '/maquinas/$slug'
      preLoaderRoute: typeof MaquinasSlugRouteImport
      parentRoute: typeof rootRouteImport
    }
    '/categorias/$slug': {
      id: '/categorias/$slug'
      path: '/categorias/$slug'
      fullPath: '/categorias/$slug'
      preLoaderRoute: typeof CategoriasSlugRouteImport
      parentRoute: typeof rootRouteImport
    }
  }
}

const rootRouteChildren: RootRouteChildren = {
  IndexRoute: IndexRoute,
  ContactoRoute: ContactoRoute,
  GarantiaSoporteRoute: GarantiaSoporteRoute,
  CategoriasSlugRoute: CategoriasSlugRoute,
  MaquinasSlugRoute: MaquinasSlugRoute,
  CategoriasIndexRoute: CategoriasIndexRoute,
  MaquinasIndexRoute: MaquinasIndexRoute,
}
export const routeTree = rootRouteImport
  ._addFileChildren(rootRouteChildren)
  ._addFileTypes<FileRouteTypes>()

import type { getRouter } from './router.tsx'
import type { startInstance } from './start.ts'
declare module '@tanstack/react-start' {
  interface Register {
    ssr: true
    router: Awaited<ReturnType<typeof getRouter>>
    config: Awaited<ReturnType<typeof startInstance.getOptions>>
  }
}
